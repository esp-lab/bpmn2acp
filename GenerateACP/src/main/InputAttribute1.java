...
public class InputAttribute1 {
	...
	private List<Artifact> artifactListtemp;
	...
	private String resultShow[] = null;
	...
	public void run() {
		...
		frmAddNewAttribute.setAlwaysOnTop(true);
	}
	...
	public void main() {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					InputAttribute1 window = new InputAttribute1(artifactList, artifactListtemp, parent,
							resultTextArea);
	 ...
	 */
	public InputAttribute1(List<Artifact> artifactList, List<Artifact> artifactListTemp, DefaultTableModel parentModel,
			JTextArea resultTextArea) {
		this.artifactList = artifactList;
		this.parent = parentModel;
		this.resultTextArea = resultTextArea;
		this.artifactListtemp = artifactListTemp;
		initialize();
	}

	public InputAttribute1(List<Artifact> artifactList, DefaultTableModel parentModel, JTextArea resultTextArea,
	...
	private void initialize() {
		...
		frmAddNewAttribute.addWindowListener(new WindowListener() {
			...
			public void windowClosed(WindowEvent e) {
				if (result != null) {
					parent.addRow(resultShow);
					resultTextArea
							.setText(((resultTextArea.getText().equals("")) ? "" : (resultTextArea.getText() + " /\\ "))
									+ resultShow[0] + resultShow[1] + resultShow[2]);
		...
		artifactCB1 = new JComboBox(artifactListtemp.toArray());
		...
		attributeCB1 = new JComboBox(artifactListtemp.get(0).getListAttribute().toArray());
		...
		btnAdd.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				String tmpShow[] = { artifactCB1.getSelectedItem() + "." + attributeCB1.getSelectedItem(),
						transOperator( operatorCB.getSelectedItem().toString()),
						("Compare to a value".equals(compareTypeCB.getSelectedItem())) ? valueTextField.getText()
								: (artifactCB2.getSelectedItem() + "." + attributeCB2.getSelectedItem()) };
				result = tmp;
				resultShow = tmpShow;
				...
				//String need=" hello ";
				//String[] a={"hello","hdhd","hhd"};
				//parent.addRow(a);
				//resultTextArea.setText(need);
		...
		frmAddNewAttribute.getRootPane().setDefaultButton(btnAdd);
	}

	public String transOperator(String operatorNeedToTrans) {

		
		switch (operatorNeedToTrans) {
		case "Equal":
			return " = ";
		case "BiggerThan":
			return " > ";
		case "EqualOrBiggerThan":
			return " >= ";
		case "SmallerThan":
			return " < ";
		default:
